...
pub trait DeviceProfileClient {
    ...
    fn bind_register_req(node_address: u64) -> Box<Future<Output=BindRegisterRsp>>;
    ...
    fn backup_bind_table_req(binding_table_entries: u16, binding_table_list: Vec<BindReq>) -> Box<Future<Output=BackupBindTableRsp>>;
    fn recover_bind_table_req(start_index: u16) -> Box<Future<Output=RecoverBindTableRsp>>;
    fn backup_source_bind_req(start_table_entries: u16, start_indes: u16, source_table_list: Vec<u64>) -> Box<Future<Output=Status>>;
    fn recover_source_bind_req(start_index: u16) -> Box<Future<Output=RecoverSourceBindRsp>>;
    // 2.4.3.3 Network Management Client Services
    fn mgmt_nwk_disc_req(scan_channels: u32, scan_duration: u8, start_index: u8) -> Box<Future<Output=MgmtNwkDiscRsp>>;
    fn mgmt_lqi_req(start_index: u8) -> Box<Future<Output=MgmtLqiRsp>>;
    fn mgmt_rtg_req(start_index: u8) -> Box<Future<Output=MgmtRtgRsp>>;
    fn mgmt_bind_req(start_index: u8) -> Box<Future<Output=MgmtBindRsp>>;
    ...
    fn mgmt_cache_req(start_index: u8) -> Box<Future<Output=MgmtCacheRsp>>;
    fn mgmt_nwk_update_req(request: MgmtNwkUpdateReq) -> Box<Future<Output=MgmtNwkUpdateNotify>>;
}
